...
(function () {
  ...
  var _config = {
    environmentScripts: !!window.__wctUseNpm ? ['stacky/browser.js', 'async/lib/async.js', 'lodash/index.js', 'mocha/mocha.js', 'chai/chai.js', '@polymer/sinonjs/sinon.js', 'sinon-chai/lib/sinon-chai.js', 'accessibility-developer-tools/dist/js/axs_testing.js', '@polymer/test-fixture/test-fixture.js'] : ['stacky/browser.js', 'async/lib/async.js', 'lodash/lodash.js', 'mocha/mocha.js', 'chai/chai.js', 'sinonjs/sinon.js', 'sinon-chai/lib/sinon-chai.js', 'accessibility-developer-tools/dist/js/axs_testing.js'],
  ...
  function setup(options) {
    ...
    if (childRunner) {
      _deepMerge(_config, childRunner.parentScope.WCT._config); // But do not force the mocha UI
  ...
  function () {
    ...
    ChildRunner.prototype.run = function (done) {
      ...
      container.appendChild(this.iframe); // let the iframe expand the URL for us.
    ...
    ChildRunner.prototype.loaded = function (error) {
      debug('ChildRunner#loaded', this.url, error); // Not all targets have WCT loaded (compatiblity mode)
    ...
    ChildRunner.prototype.done = function () {
      debug('ChildRunner#done', this.url, arguments); // make sure to clear that timeout

      this.ready();
      this.signalRunComplete();
      if (!this.iframe) return; // Be safe and avoid potential browser crashes when logic attempts to
  ...
  function () {
    ...
    CLISocket.init = function (done) {
      ...
      if (!browserId) return done(); // Only fire up the socket for root runners.
  ...
  function () {
    ...
    MultiReporter.prototype.childReporter = function (location) {
      var name = this.suiteTitle(location); // The reporter is used as a constructor, so we can't depend on `this` being
      // properly bound.
    ...
    MultiReporter.prototype.proxyEvent = function (eventName, runner) {
      ...
      debug('MultiReporter#proxyEvent(', arguments, ')'); // This appears to be a Mocha bug: Tests failed by passing an error to their
  ...
  var htmlSuites$1 = [];
  var jsSuites$1 = []; // We process grep ourselves to avoid loading suites that will be filtered.

  var GREP = getParam('grep'); // work around mocha bug (https://github.com/mochajs/mocha/issues/2070)
  ...
  function _fixCustomElements() {
    ...
    for (var i = 0; i < elements.length; i++) {
      var el = elements[i]; // This child has already been cloned and replaced by its parent, skip it!

      if (!el.isConnected) continue;
      var tag = el.localName; // Not a custom element!

      if (tag.indexOf('-') === -1) continue; // Memoize correct constructors.

      constructors[tag] = constructors[tag] || document.createElement(tag).constructor; // This one was correctly upgraded.
  ...
  function injectMocha(Mocha) {
    ...
    _injectPrototype(HTML, Mocha.reporters.HTML.prototype); // Mocha doesn't expose its `EventEmitter` shim directly, so:
  ...
  function loadSync() {
    ...
    imports.forEach(function (path) {
      var url = expandUrl(path, get('root'));
      debug('Loading environment import:', url); // Synchronous load.
  ...
  function _ensureMocha() {
    ...
    injectMocha(Mocha); // Magic loading of mocha's stylesheet

    var mochaPrefix = scriptPrefix('mocha.js'); // only load mocha stylesheet for the test runner output
  ...
  extendInterfaces('stub', function (_context, teardown) {
    return function stub(tagName, implementation) {
      ...
      var proto = document.createElement(tagName).constructor.prototype; // For all keys in the implementation to stub with..
    ...
    return function replace(oldTagName) {
      return {
        with: function (tagName) {
          ...
          replacements[oldTagName] = tagName; // If the function is already a stub, restore it to original
          ...
          var originalImportNode = document.importNode; // Use Sinon to stub `document.ImportNode`:
          ...
          sinon.stub(document, 'importNode', function (origContent, deep) {
            ...
            var inertDoc = content.ownerDocument; // imports node from inertDoc which holds inert nodes.

            templateClone.content.appendChild(inertDoc.importNode(origContent, true)); // optional arguments are not optional on IE.

            var nodeIterator = document.createNodeIterator(content, NodeFilter.SHOW_ELEMENT, null, true);
            var node; // Traverses the tree. A recently-replaced node will be put next,
            ...
            while (node = nodeIterator.nextNode()) {
              ...
              if (replacements.hasOwnProperty(currentTagName)) {
                currentTagName = replacements[currentTagName]; // find the final tag name.
                ...
                var replacement = document.createElement(currentTagName); // For all attributes in the original node..
          ...
          if (!replaceTeardownAttached) {
            // After each test...
            teardown(function () {
              replaceTeardownAttached = true; // Restore the stubbed version of `document.importNode`:
  ...
  setup(window.WCT); // Maybe some day we'll expose WCT as a module to whatever module registry you
  ...
  listenForErrors();
  stubInterfaces();
  loadSync(); // Give any scripts on the page a chance to declare tests and muck with things.

  document.addEventListener('DOMContentLoaded', function () {
    debug('DOMContentLoaded');
    ensureDependenciesPresent(); // We need the socket built prior to building its reporter.

    CLISocket.init(function (error, socket) {
      if (error) throw error; // Are we a child of another run?
      ...
      var reportersToUse = determineReporters(socket, parent); // +1 for any local tests.
      ...
      loadJsSuites(reporter, function (error) {
        ...
        if (error) throw error; // Emit any errors we've encountered up til now
