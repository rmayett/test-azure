...
(function (Mocha, chai, axs) {
    ...
    Mocha.interfaces[iface] = function (suite) {
      ...
      suite.on('pre-require', function (context, file, mocha) {
        ...
        a11ySuiteExport = context.a11ySuite = function (fixtureId, ignoredRules, beforeEach) {
          ...
          axsConfig.auditRulesToIgnore = ignoredRules; // build mocha suite

          var a11ySuite = Suite.create(suite, 'A11y Audit - Fixture: ' + fixtureId); // override the `eachTest` function to hackily create the tests
          //
          ...
          a11ySuite.eachTest = function () {
            // instantiate fixture
            fixtureElement.create(); // Make sure lazy-loaded dom is ready (eg <template is='dom-repeat'>)

            Polymer.dom.flush(); // If we have a beforeEach function, call it
            ...
            var auditResults = axs.Audit.run(axsConfig); // create tests for audit results

            auditResults.forEach(function (result, index) {
              // only show applicable tests
              if (result.result !== 'NA') {
                var title = result.rule.heading; // fail test if audit result is FAIL
  ...
  chai.use(function (chai, util) {
    var Assertion = chai.Assertion; // assert
    ...
    Assertion.addMethod('a11yLabel', function (str, msg) {
      ...
      var node = this._obj; // obj must be a Node

      new Assertion(node).to.be.instanceOf(Node); // vind the text alternative with the help of accessibility dev tools

      var textAlternative = axs.properties.findTextAlternatives(node, {});
      this.assert(textAlternative === str, 'expected #{this} to have text alternative #{exp} but got #{act}', 'expected #{this} to not have text alternative #{act}', str, textAlternative, true);
    });
  });
